...
namespace MultiBoost {
	...
	void AdaBoostMHLearner::doLikelihoods(const nor_utils::Args& args)
	{
		AdaBoostMHClassifier classifier(args, _verbose);
	...
	void AdaBoostMHLearner::doROC(const nor_utils::Args& args)
	{
		AdaBoostMHClassifier classifier(args, _verbose);
	...
	void AdaBoostMHLearner::doCalibratedPosteriors(const nor_utils::Args& args)
	{
		AdaBoostMHClassifier classifier(args, _verbose);
	...
	// -------------------------------------------------------------------------
	// -------------------------------------------------------------------------
	float AdaBoostMHLearner::updateWeights(OutputInfo* pOutInfo, InputData* pData, vector<BaseLearner*>& pWeakHypothesis){
	...
	float AdaBoostMHLearner::updateWeights(OutputInfo* pOutInfo, InputData* pData, vector<BaseLearner*>& pWeakHypothesis){
		...
		vector<BaseLearner*>::iterator it;
		//int i = 0;
		// calculate the margins ( f^{t}(x_i) )
		for( it = pWeakHypothesis.begin(); it != pWeakHypothesis.end(); it++ )
		{
			BaseLearner* pWeakHypothesis = *it;
			const float alpha = pWeakHypothesis->getAlpha();
			for (int i = 0; i < numExamples; ++i)
			{
				...
				for (lIt = labels.begin(); lIt != labels.end(); ++lIt )
				{
					_hy[i][lIt->idx] += alpha * pWeakHypothesis->classify(pData, i, lIt->idx ); // alpha * h_l(x_i)				
		...
		for (int i = 0; i < numExamples; ++i)
		{
			...
			for (lIt = labels.begin(); lIt != labels.end(); ++lIt )
			{
				_hy[i][lIt->idx] = -_hy[i][lIt->idx] * lIt->y;
				if ( mx < _hy[i][lIt->idx] ) mx = _hy[i][lIt->idx];
			}
		...
		// Now do the actual weight
		// for each example, w_i = exp( -f(x_i)*y_i ) / Z
		for (int i = 0; i < numExamples; ++i)
		{
			vector<Label>& labels = pData->getLabels(i);
			vector<Label>::iterator lIt;
	...
	*/
	...
	float AdaBoostMHLearner::updateWeights(InputData* pData, BaseLearner* pWeakHypothesis)
	{
		const int numExamples = pData->getNumExamples();
		const int numClasses = pData->getNumClasses();
